...
void updateReceiveProgress(std::ofstream& file, char* buffer, int bytes_recieved, double& MB, double fileSizeMB) {
  ...
  std::cout << "\r" << color::cyan << "Progress: " << percent  << "%" << color::reset << " [" << color::green << progressBar << color::reset << "] " 
    << color::green << std::fixed << std::setprecision(1) << MB << "/" << fileSizeMB << " MB " << color::reset << std::flush;
}
